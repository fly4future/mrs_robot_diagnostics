...
double SensorHandler::getMeasuredRate() const {
  return rate_tracker_.rate();
}

bool SensorHandler::isInGracePeriod(const rclcpp::Time &now) const {
  return (now - init_time_).seconds() < GRACE_PERIOD_S;
}

bool SensorHandler::isTopicFresh(const rclcpp::Time &now, const rclcpp::Time &last_msg) const {
